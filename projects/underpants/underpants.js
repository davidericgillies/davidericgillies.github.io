...
_.typeOf = function (any) {
    ...
    if(any instanceof Date) return 'Date';
    if(typeof any === 'object') return 'object';
    return typeof any;
};
...
_.first = function (arr, num) {
    ...
    if (isNaN(num) || num == undefined) return arr[0];
...
_.last = function (arr, num) {
    ...
    if (isNaN(num) || num == undefined) return arr[arr.length - 1];
...
_.each = function (coll, test) {
    // show me what you're working with
    // per type, iterate, access (how you get to the elements/values)
    // per value, apply the action, passing value, poisition, collection
...
_.filter = function (arr, test) {
    ...
    // show me what you're working with?
    // apply test
    //    > ask the question, did this value pass the test?
    //          > push to results
    // test is a 'client'
    const results = [];
...
_.partition = function (array, test) {
    let arr = [_.filter(array, test), _.reject(array, test)];
    return arr;
};
...
_.pluck = function (array, prop){
    let result = [];
       _.map(array, function(val, ind, coll) {
           result.push(array[ind][prop]);
       });
       return result;
};
...
_.every = function (coll, test) {
  ...
  if (typeof test !== "function") {
      _.map(coll, function(val, ind, coll) {
      if (!!val === true) {
      answer.push(true);
    } else {
      answer.push(false);
  ...
  return _.indexOf(answer, false) > -1 ? false:true;
  }
  _.map(coll, function(val, ind, coll) {
      if (test(val,ind,coll) === true ){
      answer.push(true);
    } else if (test(val,ind,coll) !== true) {
      answer.push(false);
    }
...
var answer = [];
  ...
  _.map(coll, function(val, ind, coll) {
   ...
   return _.indexOf(answer, true) <= -1 ? false:true;
};
...
_.reduce = function(array, combine, seed) {
  // create something to contain combined, or something to which you'll write your summary
  // if seed, assign to combined
  //         >else, assign combined to first value in array
  //         >AND 
  // loop through array, and combine
  // return combined
  let
      combined = seed,
      i = 0;
...
_.extend = function(obj1, obj2) {
  let args = new Array(arguments.length);
    for(var i = 0; i < args.length; ++i) {
     ...
     _.each(args, function(obj, key, arr) {
    for (var key in obj) {
      obj1[key] = obj[key];
    }
  });
